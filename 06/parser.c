...
void assemble(const char * file_name, instruction* instructions, int num_instructions) {
	char file[strlen(file_name) + 5];
	strcpy(file, file_name);
	strcat(file, ".hack");
	printf("%s\n", file);
	int new = 16;

	FILE *fnew = fopen(file, "w+");

	for (int i = 0; i < num_instructions; ++i) {
		opcode opcode;
		// If A-type
		if (instructions[i].type == Atype) {
			// If A-type label
			if (!instructions[i].instr.a.is_addr) {
				// If not in symtable, add it
				if (symtable_find(instructions[i].instr.a.operand.label) == NULL) {
					symtable_insert(instructions[i].instr.a.operand.label, new);
					opcode = new++;
				}
				// Else, pull from symtable
				else {
					opcode = symtable_find(instructions[i].instr.a.operand.label)->address;
				}
		...
		// If C-type
		else if (instructions[i].type == Ctype) {
			opcode = instruction_to_opcode(instructions[i].instr.c);
		}

		fprintf(fnew, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n", OPCODE_TO_BINARY(opcode));
	}
